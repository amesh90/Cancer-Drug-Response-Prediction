...
def RF_explain(X_train, X_test, y_train, y_test,feature_names):

	# pca = PCA(n_components=50)

	# smt = smogn.smoter(data=dfr, y='AUC')

	regr = RandomForestRegressor(
		 	criterion='mse', min_samples_split=50, min_samples_leaf=50, 
		 	max_features='log2',  oob_score=False, n_jobs=-1, 
		 	random_state=47,  max_samples=len(X_train)//2, max_depth=15, n_estimators=100)

	# pipe = Pipeline(steps=[('pca', pca), ('rf', regr)])
	...
	print('Random Forest MSError', np.mean((regr.predict(X_test) - y_test) ** 2))
	print('MSError when predicting the mean', np.mean((y_train.mean() - y_test) ** 2))

	# categorical_features = np.array([i for i in range(len(feature_names))]) #np.argwhere(np.array([len(set(boston.data[:,x])) for x in range(boston.data.shape[1])]) <= 10).flatten()
	explainer = lime.lime_tabular.LimeTabularExplainer(X_train, feature_names=feature_names, class_names=['AUC'], verbose=True, mode='regression')
	# explainer = lime.lime_tabular.LimeTabularExplainer(X_train, feature_names=feature_names, class_names=['AUC'], discretize_continuous=True)
	# i = 25
	i = np.random.randint(0, X_test.shape[0])
	exp = explainer.explain_instance(X_test[i], regr.predict, num_features=5)

	
	# exp = explainer.explain_instance(X_test[i], regr.predict, num_features=2, top_labels=1)

	a = exp.show_in_notebook(show_table=True,  show_all=True)
	print(a)
	s = exp.as_list()
	print(s)
	   ...
	   #            'svr__gamma': [ 0.0005, 0.01, 0.1], }
			...
			if model == 'RANDOM_FOREST' :
				...
				for which in whichlist:
					...
					# print(which)
					# print()
					feature_names = ['MUC12', 'C16orf86', 'PSMB1', 'ZRANB3', 'TAB2', 'RBM26', 'IST1', 'OR56B1', 'DLX6', 'LOXL4', 'CCR5', 'CYP2D6', 'OR1J1', 'SUSD1', 'TRIM29', 'EMC3', 'XPO5', 'GLT1D1', 'WDR11', 'DENND1C', 'RGS6', 'DNM3', 'SOX17', 'PRSS27', 'MGAT4B', 'LCE3D', 'NCOA3', 'CACNB1', 'ZFYVE26', 'HIST1H3A', 'ADAM18', 'SH2D4B', 'PRRG2', 'TIGD6', 'WNT11', 'PTAR1', 'MTMR12', 'MRPL55', 'GCLC', 'USP19', 'ERC1', 'PER3', 'OTUD3', 'ATHL1', 'CYP27A1', 'SOX5', 'PPM1J', 'PLD1', 'TMEM19', 'MLPH', 'TMEM2', 'SPRY3', 'MDC1', 'OAS3', 'PARK7', 'PODNL1', 'WDR96', 'MPDU1', 'CA3', 'NKIRAS1', 'ARHGEF19', 'AGPS', 'GZMA', 'CTSL1']
					
					print('\n\n ', len(feature_names) , X_train.shape)
					RF_explain(X_train, X_test, y_train, y_test,feature_names)
